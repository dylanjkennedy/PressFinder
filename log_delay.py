...
def games_to_plays (games, opponent, params):
    ...
    print ("All API Requests are made using the following python command with the requests library:")
    print ("requests.get('https://api.profootballfocus.com/v1/video/ncaa/games/GAMEID/plays', headers = " + str(params)+"'")
    print ("This test was run on " + datetime.now(pytz.timezone("America/Chicago")).strftime("%m/%d/%Y, %H:%M:%S %p") + " CT")           
    wr_dict = {}
    for game in games:
        print ("\nRequesting Gameid: " + str(game))
        start = time.time()
        r = requests.get('https://api.profootballfocus.com/v1/video/ncaa/games/'+game+'/plays', headers = params)
        end = time.time()
        print("Request took " + str(end - start) + " seconds")
        plays = r.json()['plays']
        for play in plays:
